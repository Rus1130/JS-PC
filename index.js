...
class PC {
    ...
    static #runMachineCode(register){
        let instructionStart = register.data.findIndex(byte => byte !== undefined);
        ...
        for(let i = 0; i < instructionLength; i++){
            register.unsetAddress(instructionStart + i);
        }
    }
    ...
    static instruct(array) {
        ...
        switch (instructionName) {
            case 'ram_store': {
                if (args.length !== 2) {
                    PC.halt(`Invalid number of arguments for ram_store: expected 2, got ${args.length}`, ['MACHINE CODE']);
                    return;
                }
                let address = args[0];
                let value = args[1];
            ...
            case "set_pixel": {
                ...
                let x = args[0];
                let y = args[1];
                let backColor = colorBlockVisualized()[args[2]];
                let foreColor = colorBlockVisualized()[args[3]];
                let character = characterBlockVisualized()[args[4]];
                ...
                if (cell == null) {
                    PC.halt(`Cell at coordinates (${x}, ${y}) does not exist`, ['MACHINE CODE']);
                    return;
                }
            ...
            case "subtract": {
                ...
                PC.log(`Subtracted values at addresses ${address1} and ${address2}, stored result ${subResult} at address ${outputAddresss}`, ['MACHINE CODE', 'SUCCESS']);
            } break;
